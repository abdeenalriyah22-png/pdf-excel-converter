                    ...
                    try:
                        with st.spinner('جاري تفكيك الجداول وهيكلتها...'):
                            ...
                            if dfs:
                                ...
                                st.download_button(
                                    label="📥 اضغط هنا لتحميل ملف Excel المستخرج",
...
with tab2:
    ...
    ocr_file = st.file_uploader("ارفع صورة الفاتورة/المستند (JPG, PNG) أو ملف PDF الممسوح", type=["jpg", "png", "jpeg", "pdf"], key="ocr_main")
    
    if ocr_file:
        if st.button("🚀 اطلَق الذكاء الاصطناعي لقراءة النص"):
            full_text = ""
            try:
                with st.spinner('جاري المسح الضوئي للمستند وتفسير الحروف...'):
...
st.markdown("""
    <div class="footer">
        المحاسب الذكي Pro | <span style="color:#58a6ff;">الفصل في الذمة.. الوصل في الأمانة</span> | 2026 ©
    </div>
""", unsafe_allow_html=True)
